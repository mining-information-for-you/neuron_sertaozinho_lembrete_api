...
async def get_schedule(
    ...
    data_envio: str = Query(
        None, description="Data no formato dd/mm/yyyy para filtrar data_hora_enviar"
    ),
    ...
    try:
        ...
        if data_envio:
            try:
                ...
                data_hora_enviar = (
                    parsed_date  # Sobrescrever se data_envio foi fornecida
                )
            except ValueError:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if value is not None:
                if field == "data_hora_enviar" and isinstance(value, datetime):
                    ...
                    if data_envio:
                        ...
                        end_date = value
                    conditions.append(
                        f"{field} >= ${param_index} AND {field} <= ${param_index + 1}"
                    )
...
async def update_response(
    ...
    try:
        if resposta:
            ...
            # Remove acentos
            resposta_normalized = "".join(
                [c for c in resposta_normalized if not unicodedata.combining(c)]
            )
        ...
        data = await conn.execute(query, resposta, dt_resposta, wa_message_id)
        return data
